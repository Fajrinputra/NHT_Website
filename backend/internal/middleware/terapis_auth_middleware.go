...
// TerapisAuthMiddleware validates JWT token and ensures the role is "terapis"
func TerapisAuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Verify role
		if claims.Role != "terapis" {
			utils.ResponseError(c, http.StatusForbidden, "Akses ditolak: Hanya terapis yang diizinkan")
			c.Abort()
			return
		}

		c.Set("terapisId", claims.TerapisID)
		c.Next()
	}
}
