...
export default function BayiPage() {
  ...
  const loadData = () => {
    setIsFetching(true);
    getAnak()
      .then((res) => setAnaks(res.data.data || []))
      .catch(() => setAnaks([]))
    ...
    <AppShell>
      ...
      <section className="mt-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-bold text-gray-800">Profil Anak</h2>
          <button
        ...
        {isFetching ? (
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide -mx-4 px-4 lg:mx-0 lg:px-0">
            {anaks.map((anak) => (
              <div key={anak.id} className="card min-w-[260px] flex-shrink-0 flex items-center gap-3 bg-gradient-to-br from-primary-50 to-white border border-primary-100">
      ...
      {anaks.length > 0 && (
        <section className="mt-8 space-y-3">
          ...
          <Link to="/bayi/pertumbuhan" className="block">
          ...
          <Link to="/bayi/imunisasi" className="block">
            <div className="card flex items-center gap-3 hover:shadow-card-hover cursor-pointer">
          ...
          <Link to="/bayi/denver" className="block">
            <div className="card flex items-center gap-3 hover:shadow-card-hover cursor-pointer">
          ...
          <Link to="/bayi/catatan-harian" className="block">
            <div className="card flex items-center gap-3 hover:shadow-card-hover cursor-pointer">
              <div className="w-10 h-10 rounded-xl bg-primary-50 flex items-center justify-center text-primary-600">
                <PencilLine size={20} />
              </div>
              <span className="text-sm font-medium text-gray-800 flex-1">Catatan Harian Bayi/Anak</span>
            </div>
          </Link>
        </section>
      )}
